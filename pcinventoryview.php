...
@session_start();
if(isset($_SESSION["PCIadmin"]))
{
if($_SESSION["PCIadmin"] == "PCIadmin")
{

if(isset($_POST['ddlwing']))
{

	
if($_POST['ddlwing'] != "All")
{
	$query = "SELECT s.*,w.wingname
	FROM `tblpcinventory` s
	INNER JOIN tblwing w ON s.wing = w.wingid
	WHERE s.wing= '".$_POST['ddlwing'] ."' ";
...
else
{
	...
	$query = "SELECT s.*,w.wingname
	FROM `tblpcinventory` s
	left JOIN tblwing w ON s.wing = w.wingid";;
	$resource =	mysql_query($query);
	while($array = mysql_fetch_assoc($resource)) 
	{
		$viewarray[] = $array;
...
<!--[if (gt IE 9)|!(IE)]><!--> <html lang="en" > <!--<![endif]-->
            ...
            <section>				
                <div id="container_demo" >
                    ...
                    <div id="wrapper">
						...
						<form  method = "post" action = "pcinventoryview.php" autocomplete="on"> 
					 ...
					 -->
                                <input type ="submit" name="selectedwing" value= "Go">
                                </p>
								</div>
								</center>
								</form>
  ...
  <tr>
  <th>PCID</th>
    <th>Name Officer/Officials </th>
    <th>Designation</th>
    <th data-th="Driver details"><span>Wing</span></th>
    <th>System Description</th>
    <th>Brand Name</th>
    <th>M/S Office or Related Software</th>
    <th>LCD</th>
    <th>PRINTER</th>
	<th>Scanner</th>
	<th>UPS</th>
  ...
  <tr id = "complainstatus<?=  $counter;?>">
	<td> <?= $arr['PCId'] ?></td>
    <td> <?= $arr['OfficerName'] ?></td>
	<td> <?= $arr['Designation'] ?></td>
    <td><?= $arr['wingname'] ?></td>
    <td><?= $arr['Description'] ?></td>
    <td><?= $arr['BrandName'] ?>  </td>
    <td><?= $arr['MsOfficeOrOtherSoftware'] ?> </td>
	<td><?= $arr['LCD'] ?> </td>
	<td><?= $arr['Printer'] ?> </td>
	<td><?= $arr['Scanner'] ?> </td>
	<td><?= $arr['UPS'] ?> </td>
